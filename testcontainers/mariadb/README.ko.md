...
`testcontainers/mariadb`는 integration test용 MariaDB container를 시작하고
MySQL driver와 함께 `database/sql`에서 사용할 수 있는 DSN을 반환합니다.
...
```go
import mariadbtestcontainer "github.com/bluetape4k/bluetape-go/testcontainers/mariadb"
```
...
dsn := mariadbtestcontainer.Start(ctx, t)
details := map[string]string{
    mariadbtestcontainer.DSNKey: dsn,
}
db, err := sql.Open("mysql", details[mariadbtestcontainer.DSNKey])
if err != nil {
...
MariaDB DSN만 필요하면 `Start(ctx, t)`를 사용하세요. Host lookup, mapped port,
...
if err != nil {
    t.Fatalf("mariadb details: %v", err)
}
if err := tcserver.ExportEnv(t, details, map[string]string{
    mariadbtestcontainer.DSNKey: "BLUETAPE_MARIADB_DSN",
}); err != nil {
    t.Fatal(err)
}
```
...
- DSN key는 `mariadbtestcontainer.DSNKey` (`mariadb.dsn`)로 노출합니다.
