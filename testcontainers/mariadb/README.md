...
`testcontainers/mariadb` starts a MariaDB container for integration tests and
...
Use `Start(ctx, t)` when a MariaDB DSN is enough. Use `StartServer(ctx, t)` when
...
- Exposes the data source name key as `mariadbtestcontainer.DSNKey`
  (`mariadb.dsn`).
