...
위 판정은 구현 전 계획 판정이다. 실제 Redis 8.8.1 실행에서 RESP3 포화 응답은
`-1`이 아니라 `false`였고, 작은 필터에서는 같은 item의 COUNT도 과대 집계했다.
고정 RedisBloom 소스와 독립 researcher 확인을 근거로 spec·plan의 오류 매핑과
실패 전후 count 검증을 정정했다. 새 API나 의존성을 추가하는 범위 변경은 아니다.
간접 testify를 직접 사용하면 tidy가 의존성을 승격하므로 테스트 예제도 표준 testing으로 정정했다.
최종 구현 검증과 지적 처리는 `2026-09-08-issue-534-code-review.md`에서 별도로 판정한다.
