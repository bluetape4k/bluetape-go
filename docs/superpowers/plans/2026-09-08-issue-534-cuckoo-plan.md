...
**기술:** Go 1.26, 기존 go-redis v9.20.0, 표준 testing, 기존 Redis Testcontainers helper. 새 Go 의존성 없음.
...
| `docs/lessons/2026-09-08-issue-534-cuckoo-contracts.md` | RESP·retry·review 판단 오류의 재발 방지 |
...
- [x] `cuckoo_test.go`에 아래 첫 RED를 추가한다. 같은 표에서 nil/typed-nil client,
...
- [x] `go test -p 1 -count=1 ./probabilistic/redis -run '^TestCuckooConstructor$'`를 실행해
  `undefined: NewCuckoo`를 확인한다. 무관한 compile 오류면 RED로 계산하지 않는다.
- [x] `cuckoo.go`에 아래 공개 계약을 추가하고 생성자에서 reflect의 nil 가능한 모든
...
- [x] item 65536 byte 허용/65537 거부, Capacity 4와 2^30 경계, bucket 0→2,
...
- [x] 공개 선언 직후 `gofmt -w probabilistic/redis/cuckoo*.go`와
  `golangci-lint run ./probabilistic/redis`를 실행한다. Go doc은 식별자 다음 ASCII 공백과 한국어를 사용한다.
- [x] constructor/input 회귀 GREEN을 기록한다. 실제 서버 계약 정정까지 수렴한 뒤 작업 5에서 함께 commit한다.

## 작업 2: 명령과 오류 경계 (높은 복잡도, 작업 1 이후)

- [x] mutex로 요청 slice를 복사하는 fake Do를 추가한다. 응답은 `redis.NewCmd(ctx)`에
...
- [x] 성공 응답과 argv를 표로 먼저 고정하고 아직 실패하는 결과를 확인한다.
...
- [x] 모든 메서드는 같은 전송 함수에서 pre-check → Do → Result → ctx.Err 검사 순서를
  공유한다. nil command와 잘못된 응답은 ErrCuckooReply, Add의 단일 -1/RESP3 false는 ErrCuckooFull이다.
  응답 오류가 있으면 결과를 반환하지 않고, 오류 경로에서 bool/count는 항상 zero다.
- [x] 오류 판정 순서를 아래대로 구현한다. error-plus-output와 후행 취소에서는
...
- [x] typed `redis.Error`의 unknown command, 일반 error의 같은 문자열, ACL, WRONGTYPE,
...
- [x] Reserve OK 이외, Count string/음수/float, Exists/Delete int64 2, Add 빈 배열·2개 배열·
  int64 0·string·nested error를 거부한다. RESP3 false는 삽입 거부로 처리한다.
  mutation malformed는 commit-unknown이다.
- [x] key·namespace·item·provider secret이 `%v`/`%+v`에 없고 `errors.Is/As`에는
  원인이 남는지 검증한다. unknown 결과를 성공 횟수에 넣지 않는 caller 예제를 검증한다.
- [x] `go test -p 1 -count=1 -timeout=3m ./probabilistic/redis -run '^TestCuckoo'`
  GREEN, `go test -race -p 1 -count=1 -timeout=3m ./probabilistic/redis -run '^TestCuckoo'`
  GREEN을 기록하고 commit은 작업 5에서 통합한다. 의도하지 않은 재시도는 호출 수 assertion으로 탐지한다.
...
- [x] 기존 `newRedisClient(t)`로 plain Redis 7.4의 CF.RESERVE 실패를 먼저 확인한다.
  `errors.Is(err, ErrCuckooUnsupported)`와 commit-unknown 부재를 assertion한다.
- [x] opt-in fixture는 아래 값만 사용한다. `BLUETAPE_CUCKOO_INTEGRATION=1`이 아니면
...
- [x] 기존 `testcontainers/redis.StartServer`는 image를 주입할 수 없어 이 양성 fixture에는
...
- [x] RESP2와 RESP3 각각 Reserve/Add duplicate/Count/Delete, missing key Add 거부,
  reserve 충돌, namespace 격리를 검증한다. missing/WRONGTYPE Exists=false도 검증한다.
- [x] Capacity4·BucketSize2·Expansion0에 같은 item을 최대128회 추가해 포화를 찾고
  ErrCuckooFull과 count 불변을 확인한다. 포화가 관측되지 않으면 FAIL한다.
- [x] 별도 namespace에서 Capacity512·BucketSize64·Expansion0으로 64개 goroutine이
  같은 item을 한 번씩 추가한다. WaitGroup 이후 모든 오류 nil, Count64를 assertion한다.
- [x] 응답 유실은 #611 별도 worktree의 `ratelimit/sql/provider_cutover_test.go`에서
...
- [x] 아래 명령을 직렬 실행한다. 실패는 원인을 설명하고 수정한 뒤 같은 명령으로 재검증한다.
...
- [x] fixture·통합 회귀의 GREEN을 기록하고 commit은 작업 5에서 통합한다. Docker 관련 다른 suite와 병렬 실행하지 않는다.

## 작업 4: 사용 예제·문서·전체 검증 (중간 복잡도, 작업 3 이후)

- [x] `cuckoo_example_test.go`의 fake-only `ExampleNewCuckoo`는 Reserve 후 Add 성공만
...
- [x] 두 README에 API, Namespace/item byte 계약, Reserve 신뢰 설정/quota, EXPANSION0,
...
- [x] `make fmt-check`, `make tidy-check`, `go vet ./probabilistic/redis`,
  `golangci-lint run ./probabilistic/redis`, 해당 package 전체 일반/race를 순서대로 실행한다.
- [x] `make ci`를 main의 별도 폴링 process로 실행한다. 실패 package와 원인을 기록한다.
  무관한 환경 실패는 targeted proof와 분리하며 전체 PASS로 바꾸지 않는다.
- [x] `git diff --check`, 변경 파일·untracked 검사, public Go doc·README parity를 확인한다.
  SPW-01..05와 KO-01..07을 plan/review/lesson별로 기록한다.
- [x] 여섯 관점과 main 코드 검토를 수행하고 P0–P3 지적을 모두 처리한다.
  native 실행 불가면 실패 근거를 보존하고 inline fallback으로 표시한다.

## 작업 5: 교훈·개별 PR 전달 (작업 4 이후)

- [x] 교훈에 CF helper의 zero 생략·RESP2/3, 오류와 cancellation 우선순위,
...
작업 1–4는 회귀·문서·리뷰와 전체 `make ci` 종료 코드 0을 확인했다. 최종 opt-in 일반 2.173초·race 2.564초도 통과했다. 중간 코드 commit은 실제 Redis 계약 정정과 리뷰 수렴 뒤 작업 5에서 통합한다. 작업 5의 PR 전달은 별도 최종 증거로 갱신한다.
