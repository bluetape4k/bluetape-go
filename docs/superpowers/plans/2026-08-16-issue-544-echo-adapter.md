...
- [x] **Step 4: dependency diff를 검토하고 commit한다.**
...
- [x] **Step 1: `AbortWithProblem` RED 테스트를 작성한다.**
...
- [x] **Step 3: request context RED/GREEN을 작성한다.**
...
- [x] **Step 4: format와 targeted test를 실행한다.**
...
- [x] **Step 1: allowed/rejected/backend/canceled RED 케이스를 작성한다.**
  ...
  검증한다.
- [x] **Step 2: Echo context carrier를 구현한다.**
...
- [x] **Step 3: error path와 cleanup을 구현한다.**
  ...
  책임을 문서화한다.
- [x] **Step 4: package tests/race를 실행한다.**
...
- [x] **Step 1: success/error/retry boundary RED 케이스를 작성한다.**
...
- [x] **Step 2: Echo `HandlerFunc` error contract를 연결한다.**
...
- [x] **Step 3: attempt state와 redaction을 구현한다.**
...
- [x] **Step 4: targeted/race/vet를 실행한다.**
  `go test -count=1 ./web/echo`, `go test -race -count=1 ./web/echo`,
  `go vet ./web/echo`.
...
- [x] **Step 2: compile-checked examples를 추가한다.**
...
- [x] **Step 4: 문서와 source parity를 검증한다.**
  `git diff --check`, 링크/코드 fence 확인, locale diff 검토, examples test를
  다시 실행한다.

- [x] **Step 5: request-path benchmark evidence를 기록한다.**
  `BenchmarkEchoAdapter`의 성공·실패 후 retry·replayable POST body와
  serial/parallel 행을 CPU `1`에서 `count=5`로 보존하고, CPU `1,2,4` 전체
  matrix도 PASS로 확인했다. command·환경·raw output·요약표·chart·use-case
  해석은 `docs/research/outputs/issue-544`에 보존했으며, startup 비교나
  호스트 간 회귀 결론은 이 local baseline에서 주장하지 않는다.
...
- [ ] **Step 1: final validation을 순차 실행한다.**
  ...
  `go test -run '^$' -bench '^BenchmarkEchoAdapter$' -benchtime=100ms
  -benchmem -count=5 -cpu=1 ./web/echo`, full matrix
  `go test -run '^$' -bench '^BenchmarkEchoAdapter' -benchtime=100ms
  -benchmem -count=5 -cpu=1,2,4 ./web/echo`, `make ci`, `git diff --check`를
  실행한다.
