...
- [x] 보완된 spec 사용자 재승인.
- [x] 6개 관점과 메인 통합 spec review 완료. 초기 안정성 P1은 provider `Cause` 제거 후 correction lane에서 재검토해 해소했다.
- [ ] 구현 계획 작성·검토·승인.
- [ ] 구현·검증·PR 생성·CI 확인.

현재 문서는 구현 완료 보고가 아니다. 다음 단계는 `$writing-plans`로 구현 계획을 작성하고 별도 계획 승인·검토를 받는 것이다.
