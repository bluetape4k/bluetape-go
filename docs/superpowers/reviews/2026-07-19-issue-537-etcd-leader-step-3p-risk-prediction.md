...
| Lease-level cross-principal operations | RBAC principal이 다른 election key가 붙은 lease를 revoke/keepalive하거나 권한 모델 변화로 격리가 깨진다. | B가 A candidate lease Revoke 또는 `KeepAliveOnce`에 성공하거나 authz 결과가 pinned v3.6.13 behavior와 달라진다. | Task 7 isolated authenticated fixture에서 symmetric range 권한, unattached revoke, attached revoke/keepalive denial을 검증한다. Same-range principal은 mutual trust이며 상호 불신 tenant는 항상 별도 cluster를 사용한다. | behavior drift면 design review를 재개하고 server upgrade를 hold한다. 침해된 owner는 fail closed하고 모든 보호 작업을 중단한다. | 검증: main session; RBAC/cluster 격리: operator |
