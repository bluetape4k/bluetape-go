...
The bar chart is generated from the same raw output and covers all
`compression.All()` algorithms for large JSON, text, binary, and random
payloads. It is intentionally visual-first: bar length carries the comparison
signal, while the tables and raw output remain the numeric source of truth.
