...
var (
	// ErrInvalidCoordinate는 유효하지 않은 WGS84 좌표를 나타낸다.
	ErrInvalidCoordinate = errors.New("geocoding: invalid coordinate")
	// ErrInvalidOptions는 endpoint 또는 요청 option이 유효하지 않음을 나타낸다.
	ErrInvalidOptions = errors.New("geocoding: invalid options")
	// ErrNoResult는 provider가 좌표의 결과를 찾지 못했음을 나타낸다.
	ErrNoResult = errors.New("geocoding: no result")
	// ErrProvider는 provider transport 또는 HTTP 오류를 나타낸다.
	ErrProvider = errors.New("geocoding: provider error")
	// ErrRateLimited는 provider 또는 caller rate limiter가 요청을 거부했음을 나타낸다.
	ErrRateLimited = errors.New("geocoding: rate limited")
	// ErrTimeout는 caller 또는 HTTP timeout을 나타낸다.
	ErrTimeout = errors.New("geocoding: timeout")
	// ErrParse는 provider 응답을 해석할 수 없음을 나타낸다.
	ErrParse = errors.New("geocoding: parse error")
	// ErrResponseTooLarge는 bounded response limit을 초과했음을 나타낸다.
	ErrResponseTooLarge = errors.New("geocoding: response too large")
	// ErrCache는 caller-owned cache hook 오류를 나타낸다.
	ErrCache = errors.New("geocoding: cache error")
)

// Error는 public error 문자열에 URL, payload, credential을 노출하지 않는 분류 오류다.
type Error struct {
	Kind       error
	StatusCode int
	Cause      error
...
// Error는 안정된 kind와 HTTP status만 반환한다.
...
// Unwrap는 내부 원인을 보존하되 문자열에는 포함하지 않는다.
...
// Is는 sentinel 분류와 errors.Is 호환성을 제공한다.
