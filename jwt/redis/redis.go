...
// Options는 Redis-backed JWT KeyChain repository 설정이다.
type Options = jwt.RedisRepositoryOptions

// Repository는 Redis-backed JWT KeyChain repository 구현이다.
type Repository = jwt.RedisRepository

// New는 New 공개 API의 동작을 수행하며 JWT key repository의 Redis key, TTL, serialization 계약을 보존한다.
//
// 매개변수:
