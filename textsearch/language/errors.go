...
var (
	// ErrBlankText는 textsearch language image example에서 반환값과 오류 의미를 설명한다.
	ErrBlankText = errors.New("textsearch/language: text must not be blank")
	// ErrTextTooLong는 textsearch language image example에서 반환값과 오류 의미를 설명한다.
	ErrTextTooLong = errors.New("textsearch/language: text too long")
	// ErrLanguageSubsetTooSmall는 textsearch language image example에서 설정값과 기본값 적용 방식을 설명한다.
	ErrLanguageSubsetTooSmall = errors.New("textsearch/language: at least two known languages are required")
	// ErrInvalidMinimumRelativeDistance는 textsearch language image example에서 반환값과 오류 의미를 설명한다.
