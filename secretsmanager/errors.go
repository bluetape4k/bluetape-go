...
var (
	...
	ErrNilClient = errors.New("secretsmanager: client must not be nil")
	// ErrInvalidOptions - provider option이 유효하지 않을 때 반환된다.
	ErrInvalidOptions = errors.New("secretsmanager: invalid options")
	// ErrInvalidName - secret identifier가 비어 있거나 AWS 길이 상한을 넘을 때 반환된다.
	ErrInvalidName = errors.New("secretsmanager: name is invalid")
	// ErrLookupFailed - Secrets Manager 호출이 실패했을 때 반환된다.
	ErrLookupFailed = errors.New("secretsmanager: lookup failed")
	// ErrMalformedOutput - SDK 응답의 필수 구조가 유효하지 않을 때 반환된다.
	ErrMalformedOutput = errors.New("secretsmanager: malformed response")
	// ErrMissingValue - SDK 응답에 secret value가 없을 때 반환된다.
	ErrMissingValue = errors.New("secretsmanager: value is missing")
	// ErrCacheFailed - caller-owned cache 호출이 실패했을 때 반환된다.
	ErrCacheFailed = errors.New("secretsmanager: cache operation failed")
)

// Error - secret identifier와 provider payload를 노출하지 않는 오류다.
type Error struct {
	kind      error
	operation string
	cause     error
...
// GoString 메서드는 %#v 형식에서도 secret identifier와 provider cause를 숨긴다.
