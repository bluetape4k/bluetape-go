...
func RequireInRange[T cmp.Ordered](name string, value, lower, upper T) error {
	if lower > upper {
		return fmt.Errorf("%s range is invalid: lower %v must be <= upper %v", name, lower, upper)
	}
	if value < lower || value > upper {
		return fmt.Errorf("%s[%v] must be in range [%v, %v]", name, value, lower, upper)
	}
	return nil
}
...
func RequirePositive[T Number](name string, value T) error {
	var zero T
	if value <= zero {
		return fmt.Errorf("%s[%v] must be positive", name, value)
	}
	return nil
}
...
func RequireNonNegative[T Number](name string, value T) error {
	var zero T
	if value < zero {
		return fmt.Errorf("%s[%v] must be non-negative", name, value)
	}
	return nil
}
