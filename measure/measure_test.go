...
func TestMeasureOperations(t *testing.T) {
	...
	if _, err := meters.DivScalar(0); !errors.Is(err, measure.ErrDivideByZero) {
...
func TestDurationHelpers(t *testing.T) {
	...
	tooLarge := measure.Must(float64(math.MaxInt64)/float64(time.Millisecond)+1, measure.TimeMillisecond())
	if _, err := measure.Duration(tooLarge); !errors.Is(err, measure.ErrInvalidMeasure) {
