...
func WithinBounds(table, column string, minX, minY, maxX, maxY float64, srid int) (sqlkit.Statement, error) {
	...
	polygon := envelopeWKB(minX, minY, maxX, maxY)
	return sqlkit.NewStatement(fmt.Sprintf("SELECT * FROM %s WHERE MBRContains(ST_GeomFromWKB(?, %d, 'axis-order=long-lat'), %s)", quotedTable, srid, quotedColumn), polygon), nil
}

func envelopeWKB(minX, minY, maxX, maxY float64) []byte {
	// WKB Polygon: byte order, type, ring count, point count and five points.
