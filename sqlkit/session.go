...
// Execer는 *sql.DB와 *sql.Tx가 공유하는 context-aware Exec boundary다.
type Execer interface {
	ExecContext(ctx context.Context, query string, args ...any) (stdsql.Result, error)
}

// Queryer는 *sql.DB와 *sql.Tx가 공유하는 context-aware Query boundary다.
type Queryer interface {
	QueryContext(ctx context.Context, query string, args ...any) (*stdsql.Rows, error)
}

// QueryRower는 *sql.DB와 *sql.Tx가 공유하는 context-aware QueryRow boundary다.
...
// Beginner는 database/sql transaction을 시작한다. *sql.DB가 이 interface를 구현한다.
