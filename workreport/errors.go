...
// ErrUnknownFailurePolicy 변수 공개 값이며 work report 상태, failure policy, child report 계약을 보존한다.
// 호출자는 이 식별자를 오류, 상태, 이벤트, 옵션, 또는 기본값 계약을 비교할 때 사용한다.
var ErrUnknownFailurePolicy = errors.New("unknown failure policy")

// FailurePolicyError struct 공개 타입이며 work report 상태, failure policy, child report 계약을 보존한다.
...
// Is Is 공개 API의 동작을 수행하며 work report 상태, failure policy, child report 계약을 보존한다.
//
// 매개변수:
