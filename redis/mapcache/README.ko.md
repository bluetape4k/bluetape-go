...
`Client`는 `Get`, `Set`, `SetNX`, `Del`, `Eval`만 포함한 caller-owned interface입니다.
...
`redisbucket`과 같은 value/context 계약을 사용합니다. Lua operation은 하나의
entry만 atomic하게 처리하므로 다른 entry를 lock하지 않습니다.
...
않습니다. Mutation error나 malformed Lua result는 commit-unknown이며 성공 response
뒤 cancellation은 context error만 반환하고 재시도·보상하지 않습니다.
...
Testcontainers fixture로 entry별 expiry와 CAS를 검증하며 다른 Docker test와
직렬 실행하세요.
