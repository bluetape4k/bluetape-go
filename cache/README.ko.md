...
`cache`는 framework에 묶이지 않는 cache contract와 process-local TTL 구현을
...
- `Memory.GetOrLoad`는 하나의 cache instance 안에서 같은 key에 대한 동시 loader 호출을 하나로 합칩니다.
- Cross-process invalidation과 stampede protection은 `cache/redisnear`와 `cache/rediscoord`가 담당합니다.
